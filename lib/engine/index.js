...
Engine.prototype.renderLayout = function(context, innerResult){
  let layout = context.layout
  if(!layout) return innerResult;
  context.layout = undefined;
  if(layout[0] === '/') layout = '.' + layout;
  const template = path.resolve(this.cfg.root[0], layout);
  var engine = new Engine(Object.assign({}, this.cfg, {
    template
  }));

  /** @type {string} */
  var result = engine.render(context);
  return result.replace('$screen_placeholder', innerResult);
};
////////////////////////////
...
Engine.prototype.render = function(context) {
  ...
  if (result.stats === STATS.SUCCESS || result.stats === STATS.STOP) {
    result.value = this.renderLayout(context, result.value)
